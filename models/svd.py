...
# 电影信息数据集
ALL_MOVIES_PATH = './data/all_movies.csv'
# 评分数据集
...
class RecModel:
    ...
    def preprocess(self, full_process=False):
        ...
        if full_process:
            ...
            data = pd.read_csv(RATING_PATH, usecols=['userId', 'movieId', 'rating'])
            # 已获取title的电影
            ...
            # data.to_csv('./ml-25m/ratings_small.csv', index=False)
            self.data = data
            self.movie_dict = md_links_cn_new3_title.set_index('movieId')['title'].to_dict()
        else:
            self.data = pd.read_csv(RATING_PATH, usecols=['userId', 'movieId', 'rating'])
    ...
    # 使用 SVD 模型来为指定的用户生成电影推荐列表，根据预测的评分排序，选择前 n 部电影作为推荐结果
